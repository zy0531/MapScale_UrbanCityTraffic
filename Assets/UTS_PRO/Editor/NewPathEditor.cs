...
public class NewPathEditor : Editor
{
	...
	public void OnSceneGUI()
	{
		...
		if(!_NewPath.exit)
		{
    		...
    		if (Physics.Raycast(ray, out hit, 3000, 255)) 
    		{
    ...
    public override void OnInspectorGUI()
    {
    	...
    	if(GUILayout.Button("Finish"))
    	{
    		if(!_NewPath.errors)
    		{
                ...
                else if(_NewPath.PathType == PathType.VehiclePath)
                {
                    wp = _NewPath.gameObject.AddComponent<CarWalkPath>() as WalkPath;
                }
                else
                {
                    wp = _NewPath.gameObject.AddComponent<BcycleGyroPath>() as WalkPath;
                }
