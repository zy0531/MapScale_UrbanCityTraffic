...
public class StandingPeopleStreet : MonoBehaviour {
	...
	{	
		...
		for(int i = 0; i < one; i++)
		{
			...
			{
				var obj = Instantiate(peoplePrefabs[peoplePrefabIndexes[peoplePrefabCount]], randomPosition, Quaternion.identity) as GameObject;
			    peoplePrefabCount++;

			    RaycastHit hit;

        		if(Physics.Raycast(obj.transform.position + Vector3.up * 0.1f, Vector3.down, out hit))
        		{
		...
		for(int i = 0; i < two / 2; i++)
		{
			...
			if(randomPosition != Vector3.zero)
			{
				...
				if(randomPos1 != Vector3.zero && randomPos2 != Vector3.zero)
				{
					...
					obj = Instantiate(peoplePrefabs[prefabNum], randomPos1, Quaternion.identity) as GameObject;
					...
					obs = Instantiate(peoplePrefabs[prefabNum], randomPos2, Quaternion.identity) as GameObject;

			       	RaycastHit hit1;

        			if(Physics.Raycast(obs.transform.position + Vector3.up * 0.1f, Vector3.down, out hit1))
        			{
            			obs.transform.position = new Vector3(obs.transform.position.x, hit1.point.y, obs.transform.position.z);
		...
		for(int i = 0; i < three / 3; i++)
		{
			...
			if(randomPosition != Vector3.zero)
			{
				...
				if(randomPos0 != Vector3.zero)
				{
				var obj = Instantiate(peoplePrefabs[prefabNum], randomPos0, Quaternion.identity) as GameObject;
				...
				var obs = Instantiate(peoplePrefabs[prefabNum], randomPos1, Quaternion.identity) as GameObject;
				...
				if(randomPos0 != Vector3.zero)
				{
				var obl = Instantiate(peoplePrefabs[prefabNum], randomPos2, Quaternion.identity) as GameObject;

			       RaycastHit hit2;

        		if(Physics.Raycast(obl.transform.position + Vector3.up * 0.1f, Vector3.down, out hit2))
        		{
            		obl.transform.position = new Vector3(obl.transform.position.x, hit2.point.y, obl.transform.position.z);
